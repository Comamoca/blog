    ...
    <div class="flex flex-grow border(t gray-200) w-full h-20 max-w-md md:min-w-10">
      ...
      >
        <div class="flex flex-col ${og.image ? "basis-3/4" : "basis-full"}">
          <h3 class="grow !text-xs md:!text-base px-3 pt-2 py-0 h-20 !mt-0 !mb-0 font-bold tracking-tight text-gray-900 overflow-hidden text-ellipsis">
            ${title}
          </h3>
          <p class="font-xs px-3 pb-2 !mb-0 text-gray-700 text-sm max-w-48 md:max-w-full overflow-hidden whitespace-nowrap text-ellipsis">
            ${name}
          </p>
        </div>
        <div>
