...
export async function fetchOGInfo(
  ...
  try {
    ...
    const title = doc?.title || _url.hostname || "Unknown Site";
    ...
    // Ensure siteTitle is always present
    const safeOg = {
      siteTitle: title,
      ...og,
    };
    ...
    // Log error for debugging but provide fallback data
    logger.error(
      `[fetchOGInfo] Error fetching OGP data for ${url}: ${error.message}`,
    );
