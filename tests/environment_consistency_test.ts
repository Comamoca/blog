...
let originalDisableLinkcard: string | undefined;
...
const environmentState = {
  ...
  initialize() {
    try {
      ...
      originalDisableLinkcard = Deno.env.get("DISABLE_LINKCARD");
    } catch {
      // If permission denied, use defaults
      originalRelease = undefined;
      originalDisableLinkcard = undefined;
    }
    this.original.RELEASE = originalRelease || "";
    this.original.DISABLE_LINKCARD = originalDisableLinkcard || "";
  },
  ...
  restore() {
    ...
    try {
      if (originalRelease) {
        Deno.env.set("RELEASE", originalRelease);
      } else {
        Deno.env.delete("RELEASE");
      }

      if (originalDisableLinkcard) {
        Deno.env.set("DISABLE_LINKCARD", originalDisableLinkcard);
      } else {
...
Deno.test("Environment Consistency Verification", async (t) => {
  ...
  try {
    await t.step(
      "should have identical HTML structure across environments",
      () => {
        const testData = {
        ...
        try {
          ...
          assertEquals(
            devHTML,
            prodHTML,
            "HTML structure must be identical across environments",
          );
          assertEquals(
            devHTML.includes("flex flex-grow"),
            true,
            "Layout classes should be consistent",
          );
          assertEquals(
            prodHTML.includes("flex flex-grow"),
            true,
            "Layout classes should be consistent",
          );
        } finally {
    ...
    await t.step("should maintain consistent TailwindCSS classes", () => {
      ...
      try {
        ...
        responsiveClasses.forEach((className) => {
          assertEquals(
            devHTML.includes(className),
            true,
            `Dev should include ${className}`,
          );
          assertEquals(
            prodHTML.includes(className),
            true,
            `Prod should include ${className}`,
          );
          assertEquals(
            devHTML.includes(className),
            prodHTML.includes(className),
    ...
    await t.step("should respect disable flag consistently", () => {
      try {
        ...
        assertEquals(
          prodDisabled.length,
          0,
          "Prod should respect disable flag",
        );
        assertEquals(
          devDisabled.length,
          prodDisabled.length,
          "Disable behavior should be consistent",
        );
      } finally {
        environmentState.restore();
      }
    ...
    await t.step("should maintain consistent image handling", () => {
      ...
      testCases.forEach((testData) => {
        try {
          environmentState.setDevelopment();
          const devHTML = generateLinkCardHTML(
            testData,
            "https://image-test.com",
          );

          environmentState.setProduction();
          const prodHTML = generateLinkCardHTML(
            testData,
            "https://image-test.com",
          );
          ...
          assertEquals(
            devHTML,
            prodHTML,
            `Image handling should be consistent for: ${testData.title}`,
          );

          const hasImage = testData.image !== undefined;
          assertEquals(
            devHTML.includes("<img"),
            hasImage,
            `Dev image presence should match data for: ${testData.title}`,
          );
          assertEquals(
            prodHTML.includes("<img"),
            hasImage,
            `Prod image presence should match data for: ${testData.title}`,
          );
        } finally {
          environmentState.restore();
        }
    ...
    await t.step("should provide consistent accessibility features", () => {
      ...
      try {
        ...
        a11yFeatures.forEach((feature) => {
          assertEquals(
            devHTML.includes(feature),
            true,
            `Dev should include a11y feature: ${feature}`,
          );
          assertEquals(
            prodHTML.includes(feature),
            true,
            `Prod should include a11y feature: ${feature}`,
          );
        });
      } finally {
        environmentState.restore();
