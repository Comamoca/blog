...
- Current spec directory:
  !`bash -c 'ls -la .kiro/specs/$1/ 2>/dev/null || echo "No spec directory found"'`
...
- If not exist, stop with message: "Run `/kiro:spec-requirements $1` first to
...
- Identify existing testing patterns (file placement, frameworks, mocking
  approaches)
- Map API client, database, and authentication implementation approaches
  currently used
...
Generate analysis in the language specified in spec.json (check
...
**CRITICAL**: This is an analysis phase. Provide information and options, not
final implementation decisions. The design phase will make strategic choices
...
**Generate design based on analysis:** Run `/kiro:spec-design $1` to create
technical design document

**Auto-approve and proceed:**\
Run `/kiro:spec-design $1 -y` to auto-approve requirements and generate design
directly
